...
struct ipv4_hdr *
nf_pkt_ipv4_hdr(struct rte_mbuf *pkt) {
...
struct tcp_hdr *
nf_pkt_tcp_hdr(struct rte_mbuf *pkt) {
    struct tcp_hdr *tcp;
    struct ipv4_hdr *ipv4 = nf_pkt_ipv4_hdr(pkt);
...
struct udp_hdr *
nf_pkt_udp_hdr(struct rte_mbuf *pkt) {
    struct udp_hdr *udp;
    struct ipv4_hdr *ipv4 = nf_pkt_ipv4_hdr(pkt);
...
void *
nf_pkt_payload(struct rte_mbuf *pkt, int *payload_size) {
...
int
nf_pkt_is_tcp(struct rte_mbuf* pkt) {
    return nf_pkt_tcp_hdr(pkt) != NULL;
}

int
nf_pkt_is_udp(struct rte_mbuf* pkt) {
    return nf_pkt_udp_hdr(pkt) != NULL;
}

int
nf_pkt_is_ipv4(struct rte_mbuf* pkt) {
    return nf_pkt_ipv4_hdr(pkt) != NULL;
}


void nf_pkt_print_ether(struct ether_hdr* hdr) {
...
void
nf_pkt_print_ipv4(struct ipv4_hdr* hdr) {
...
void
nf_pkt_print_tcp(struct tcp_hdr* hdr) {
...
void
nf_pkt_print_udp(struct udp_hdr* hdr) {
//    printf("Source Port??????: %u\n", hdr->src_port);
    printf("Destination Port: %" PRIu16 "\n", rte_be_to_cpu_16(hdr->src_port));
    printf("Destination Port: %" PRIu16 "\n", rte_be_to_cpu_16(hdr->dst_port));
...
void
nf_pkt_print(struct rte_mbuf* pkt) {
    struct ipv4_hdr* ipv4 = nf_pkt_ipv4_hdr(pkt);
    if (likely(ipv4 != NULL)) {
        nf_pkt_print_ipv4(ipv4);
    }

    struct tcp_hdr* tcp = nf_pkt_tcp_hdr(pkt);
    if (tcp != NULL) {
        nf_pkt_print_tcp(tcp);
    }

    struct udp_hdr* udp = nf_pkt_udp_hdr(pkt);
    if (udp != NULL) {
        nf_pkt_print_udp(udp);
    }
}