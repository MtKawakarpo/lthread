...
//	}
    ...
    if (rte_eal_remote_launch(flow_director_thread, (void *) port_info1, RX_EXCLUSIVE_LCORE) == -EBUSY) {
    ...
    //TODO: register which nfs belong to which Agent

//	for( i = 0;i< rx_thread_num ;i++){
//		cur_lcore = rte_get_next_lcore(cur_lcore, 1, 1);
////		printf("launching rx %d port %d on core %d\n", i, rx[i]->port, cur_lcore);
...
//	}
    ...
    for(i = 0; i<nb_agents; i++){
        int core_cnt = (agents_info_data[i].core_mask_count&255);
        printf("launch Agent %d of %d scheduler\n", i, core_cnt);
        for(j = 0;j<2;j++){
//            cur_lcore = agents_info_data[i].core_list[j];
            cur_lcore = rte_get_next_lcore(cur_lcore, 1, 1);

            printf(">on core %d\n", cur_lcore);
            if(rte_eal_remote_launch(sched_spawner, NULL, cur_lcore) == -EBUSY){
                printf("core %d cannot use for Agent %d sched %d\n", cur_lcore, i, j);
                return -1;
            }
        }
    }
    printf(">>>finish launch scheduler\n");

//    for(i = 0;i<tx_thread_num;i++){
//        cur_lcore = rte_get_next_lcore(cur_lcore, 1, 1);
    if (rte_eal_remote_launch(lcore_tx_main, (void *) tx[i], TX_EXCLUSIVE_LCORE) == -EBUSY) {
